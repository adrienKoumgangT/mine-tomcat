package com.github.adrien.koumgang.minetomcat.apps.server.repository;
...
public class ServerEventLogRepository extends BaseRepository implements ServerEventLogDao {
     ...
     */
    @Override
    public Optional<ServerEventLog> findById(String id) {
        if (!MongoAnnotationProcessor.isValidObjectId(id)) {
            return Optional.empty();
        }

        ObjectId objectId = new ObjectId(id);
        Document document = serverEventLogCollection.find(Filters.eq("_id", objectId)).first();
    ...
    @Override
    public List<ServerEventLog> findAll(int page, int pageSize) {
        if (page < 1) page = 1;
     ...
     */
    @Override
    public String save(ServerEventLog serverEventLog) {
        ...
        return StringIdConverter.getInstance().fromObjectId(id);
    }
    ...
    public boolean delete(String id) {
        ...
        ObjectId objectId = new ObjectId(id);
        DeleteResult result = serverEventLogCollection.deleteOne(Filters.eq("_id", objectId));
